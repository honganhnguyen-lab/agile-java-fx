...
public class NewUserStoryForm extends JFrame implements BaseComponent {
    ...
    public void init() {
        ...
        businessValField = new JComboBox<>(businessValList);
    ...
    public UserStory getUserStoryObject() {
        ...
        Integer businessVal = (Integer) businessValField.getSelectedItem();
        Double points = (Double) pointsCombo.getSelectedItem();
        String status = "new";

        UserStoryFactory userStoryFactory = UserStoryFactory.getInstance();

        UserStory userStory = userStoryFactory.createNewUserStory(name, description, points, businessVal, status);
